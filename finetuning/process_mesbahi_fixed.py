...
def main():
    ...
    for file_path in tqdm(audio_files, desc="Processing Surahs"):
        ...
        # AGGRESSIVE SPLITTING: top_db=25 to catch silence in noisy files
        # frame_length=2048, hop=512 default.
        intervals = librosa.effects.split(y, top_db=25, frame_length=2048, hop_length=512)
        
        # Fallback: If 002.mp3 (3 hours) is still 1 chunk, Force Fixed Splitting
        if len(intervals) < 10 and len(y)/sr > 600: # If > 10 mins and < 10 splits
             print(f"Fallback: {filename} has poor silence. Using fixed 30s windows.")
             # Create fixed intervals
             intervals = []
             win = 30 * sr
             stride = 30 * sr # No overlap to avoid dupes? Or slight overlap?
             # If we align perfectly, contiguous is best.
             curr = 0
             while curr < len(y):
                 end = min(curr + win, len(y))
                 intervals.append((curr, end))
                 curr += stride
        else:
             print(f"Split {filename} into {len(intervals)} chunks (Adaptive).")

        cursor_gt = 0 
        
        for (start, end) in intervals:
            # Sub-split constraint logic
            chunk_len = end - start
            max_len = 30 * sr
